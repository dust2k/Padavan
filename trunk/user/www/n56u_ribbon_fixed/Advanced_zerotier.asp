...
<tr>
											<th width="30%" style="border-top: 0 none;">在路由上启用ZeroTier Moon服务器</th>
											<td style="border-top: 0 none;">
													<div class="main_itoggle">
...
<tr><th>ZeroTier Moon服务器 公网IP or 域名</th>
				<td>
					<input type="text" class="input" name="zerotiermoon_ip" id="zerotiermoon_ip" style="width: 200px" value="<% nvram_get_x("","zerotiermoon_ip"); %>" />
					<br>如果没有填写，将使用Wan获得的IP（请注意必须为静态公网IP）；如果填写IP地址，将使用该IP（请注意必须为静态公网IP）；如果填写域名，将使用非DDNS的域名获得IP（请注意为公网IP）。
				</td>
			</tr>
<tr><th>客户端加入ZeroTier Moon服务器 ID</th>
				<td>
					...
					<br>客户端可以加入自己的服务器启用后自动生成Moon服务器的ID，也可以使用其他的moon服务器，在加入Moon时请使用客户端zerotier-cli orbit <该ID> <该ID>。
				</td>
			</tr>
										<tr>
											<th>zerotier官网</th>
